...
| PMPRB (Patented Medicine Prices Review Board) | Investigated, deferred | Checked 2026-09-26: no data on open.canada.ca (the `pmprb-cepmb` organization's 16 records are reports as HTML or PDF). Annual report and NPDUIS study tables (CompassRx, Meds Entry Watch, Meds Pipeline Monitor) are HTML and PDF, once a year; the only spreadsheet is the CompassRx 10th edition data companion (five top-50 tables). The patented medicines list stops at 2021. No module. [Details](docs/findings/specialized-federal-sources.md#pmprb-patented-medicine-prices-review-board) |
...
| Financial Consumer Agency of Canada (FCAC) | Shipped | Shipped 2026-09-26: `modules/fcac/` (4 tools: `fcac_search_credit_cards`, `fcac_get_credit_card`, `fcac_search_bank_accounts`, `fcac_get_bank_account`) over FCAC's Credit Card and Account Comparison Tools, read live in EN/FR (95 credit cards, 61 chequing and 38 savings accounts in Ontario, with every fee and rate). FCAC's 36 CKAN records are reports only; survey microdata is on request and the well-being dashboard is Power BI. [Details](docs/findings/specialized-federal-sources.md#financial-consumer-agency-of-canada-fcac) |
...
| Government of Canada Recalls and Safety Alerts | Shipped | Shipped 2026-09-26: `modules/recalls/` (3 tools) on the site's daily bilingual open-data dump (34,131 Health Canada, CFIA and Transport Canada notices, archived included) for search and counts, plus each notice's page for affected products, lots and what to do. [Details](docs/findings/specialized-federal-sources.md#government-of-canada-recalls-and-safety-alerts) |
| PHAC Health Infobase | Shipped | Shipped 2026-09-26: `modules/phac_infobase/` (3 tools: list, describe, query by column values, province and date range) over 55 curated dashboard data files (respiratory viruses and FluWatch+, wastewater, opioid and stimulant harms, measles, mpox, TB, notifiable diseases, vaccine safety, archived COVID-19); CCDSS and the current CCDI tool publish no files. [Details](docs/findings/specialized-federal-sources.md#phac-health-infobase) |
