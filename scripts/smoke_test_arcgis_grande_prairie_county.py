"""Live smoke test for the County of Grande Prairie ArcGIS Hub client."""
...
async def main() -> int:
    ...
    if not search.items:
        print("FAIL: County of Grande Prairie returned no search results at all")
        return 1

    item = search.items[0]
    detail = await _check("get_dataset", client.get_dataset(item.id))
    if not detail.download_urls:
        print("FAIL: County of Grande Prairie item has no download links")
        return 1

    if detail.service_url:
        rows = await _check("query_feature_layer", client.query_feature_layer(item.id, limit=2))
        if not rows.rows:
            print("FAIL: County of Grande Prairie query_feature_layer returned no rows")
            return 1
    else:
    ...
    print(f"County of Grande Prairie total matches for {search.query!r}: {search.total_count}")
    print("ARCGIS-GRANDE_PRAIRIE_COUNTY SMOKE TEST PASSED")
    return 0
