...
$null = Invoke-Checked "ruff check" "uv" @("run", "ruff", "check", "src", "tests", "scripts")
$null = Invoke-Checked "ruff format --check" "uv" @("run", "ruff", "format", "--check", "src", "tests", "scripts")
...
# Discovered by glob rather than listed by hand, so a new source's smoke
# test is never silently left out of the release gate.
$liveSmokeScripts = Get-ChildItem -Path "scripts" -Filter "smoke_test*.py" |
    Sort-Object Name |
