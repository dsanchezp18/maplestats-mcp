"""Live smoke test for the Saskatchewan GeoHub ArcGIS Hub client."""
...
async def main() -> int:
    ...
    if not detail.download_urls:
        print("FAIL: Saskatchewan item has no download links")
        return 1

    if detail.service_url:
        rows = await _check("query_feature_layer", client.query_feature_layer(item.id, limit=2))
        if not rows.rows:
            print("FAIL: Saskatchewan query_feature_layer returned no rows")
            return 1
    else:
