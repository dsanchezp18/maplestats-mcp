"""HTTP client for Kitchener GeoHub, an ArcGIS Hub deployment.
...
from maple_data_mcp.modules.arcgis_kitchener import constants
...
async def search_datasets(
    ...
    *,
    ...
    """Search Kitchener's ArcGIS Hub dataset catalogue; ``lang`` is accepted for consistency."""
    del lang
    ...
    cache_key = f"arcgis-kitchener:search:{query}:{tag}:{item_type}:{limit}:{offset}"
    ...
    return DatasetSearchResult(
        ...
        provenance=make_provenance(
            ...
            cached=was_cached,
            schema_name="arcgis_kitchener.DatasetSearchResult",
...
async def get_dataset(item_id: str, lang: str = "en") -> ItemDetail:
    """Get one Kitchener dataset item's metadata and download links; ``lang`` is a documented no-op."""
    del lang
    ...
    feature, was_cached = await cached_fetch(
        f"arcgis-kitchener:get_item:{item_id}", constants.CACHE_TTL_ITEM_SECONDS, fetch
    )
    props = feature.get("properties") or {}
    ...
    return ItemDetail(
        ...
        provenance=make_provenance(
            ...
            cached=was_cached,
            schema_name="arcgis_kitchener.ItemDetail",
        ),
    )
...
async def query_feature_layer(
    ...
    *,
    ...
    """Query rows from one Kitchener FeatureServer/MapServer layer; ``lang`` is accepted for consistency.
    ...
    cache_key = (
        f"arcgis-kitchener:query:{item_id}:{resolved_layer_index}:{where_clause}:{out_fields}:"
    ...
    return FeatureQueryResult(
        ...
        provenance=make_provenance(
            ...
            cached=was_cached,
            schema_name="arcgis_kitchener.FeatureQueryResult",
            limits=(
