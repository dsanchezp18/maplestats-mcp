"""HTTP client for Government of New Brunswick Open Data's Socrata (SODA) platform.
...
SODA resource API's plain list-of-row-objects shape.
"""
...
from maple_data_mcp.modules.socrata_nb import constants
...
async def search_datasets(
    ...
    *,
    ...
    """Search New Brunswick's dataset catalogue; ``lang`` is accepted for consistency."""
    del lang
    ...
    cache_key = f"socrata-nb:search:{query}:{category}:{tag}:{limit}:{offset}"
    ...
    return DatasetSearchResult(
        ...
        provenance=make_provenance(
            ...
            cached=was_cached,
            schema_name="socrata_nb.DatasetSearchResult",
...
async def get_dataset(dataset_id: str, lang: str = "en") -> DatasetDetail:
    ...
    obj, was_cached = await cached_fetch(
        f"socrata-nb:get_view:{dataset_id}", constants.CACHE_TTL_DATASET_SECONDS, fetch
    )
    ...
    return DatasetDetail(
        ...
        provenance=make_provenance(
            ...
            cached=was_cached,
            schema_name="socrata_nb.DatasetDetail",
        ),
    )
...
async def query_dataset_rows(
    ...
    *,
    ...
    cache_key = f"socrata-nb:rows:{dataset_id}:{select}:{where}:{order}:{q}:{limit}:{offset}"
    ...
    return RowQueryResult(
        ...
        provenance=make_provenance(
            ...
            cached=was_cached,
            schema_name="socrata_nb.RowQueryResult",
...
async def list_categories(lang: str = "en") -> CategoryList:
    ...
    raw, was_cached = await cached_fetch(
        "socrata-nb:domain_categories", constants.CACHE_TTL_FACET_SECONDS, fetch
    )
    categories = [
    ...
    return CategoryList(
        ...
        provenance=make_provenance(
            ...
            cached=was_cached,
            schema_name="socrata_nb.CategoryList",
        ),
    )
...
async def list_tags(lang: str = "en") -> TagList:
    ...
    raw, was_cached = await cached_fetch(
        "socrata-nb:domain_tags", constants.CACHE_TTL_FACET_SECONDS, fetch
    )
    tags = [
    ...
    return TagList(
        ...
        provenance=make_provenance(
            ...
            cached=was_cached,
            schema_name="socrata_nb.TagList",
        ),
    )
