"""MCP tools for the City of Windsor's ArcGIS Hub open-data portal."""
...
from maple_data_mcp.modules.arcgis_windsor import client
from maple_data_mcp.modules.arcgis_windsor.constants import ROWS_LIMIT_DEFAULT, SEARCH_LIMIT_DEFAULT
from maple_data_mcp.modules.arcgis_windsor.schemas import (
    DatasetSearchResult,
    FeatureQueryResult,
    ItemDetail,
)
...
@tool
async def arcgis_windsor_search_datasets(
    query: str = "",
    tag: str | None = None,
    ...
    """Search Windsor's ArcGIS Hub open-data catalogue (Windsor Open Data Portal).

    Use for: finding Windsor datasets by topic, tag, item type, or
...
@tool
async def arcgis_windsor_get_dataset(item_id: str, lang: Lang = "en") -> ItemDetail:
    """Get one Windsor dataset item's metadata, service URL, and download links.

    Use for: inspecting a dataset found with arcgis_windsor_search_datasets
...
@tool
async def arcgis_windsor_query_feature_layer(
    item_id: str,
    ...
    """Query rows from one Windsor FeatureServer/MapServer layer.

    Use for: reading actual feature attribute values (not just
    metadata) from a dataset found with arcgis_windsor_search_datasets,
