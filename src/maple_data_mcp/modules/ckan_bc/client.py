...
async def datastore_search(
    ...
    *,
    ...
    (from ckan_bc_get_dataset/ckan_bc_get_resource) before calling this.
    Confirmed live this is exactly how BC's Foundation Skills Assessment
    district-level results (grade/subject/year/average score) are
    reachable row-by-row rather than only as one bulk CSV download.
    """
    del lang
    if not resource_id.strip():
        raise InvalidInput("resource_id must not be empty.")
    ...
    cache_key = (
        f"ckan-bc:datastore_search:{resource_id}:{filters}:{query}:{sort}:{fields}:{limit}:{offset}"
    )
    ...
    return DatastoreSearchResult(
        ...
        provenance=make_provenance(
            ...
            cached=was_cached,
            schema_name="ckan_bc.DatastoreSearchResult",
