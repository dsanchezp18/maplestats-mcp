"""MCP tools for Prince Edward Island's ArcGIS Hub open-data portal."""
...
from maple_data_mcp.modules.arcgis_pe.constants import ROWS_LIMIT_DEFAULT, SEARCH_LIMIT_DEFAULT
from maple_data_mcp.modules.arcgis_pe.schemas import (
    DatasetSearchResult,
    FeatureQueryResult,
    ItemDetail,
)
...
@tool
async def arcgis_pe_search_datasets(
    query: str = "",
    tag: str | None = None,
    ...
    """Search Prince Edward Island's ArcGIS Hub open-data catalogue (PEI Open Data Portal).

    Use for: finding Prince Edward Island datasets by topic, tag, item type, or
...
@tool
async def arcgis_pe_get_dataset(item_id: str, lang: Lang = "en") -> ItemDetail:
    """Get one Prince Edward Island dataset item's metadata, service URL, and download links.

    Use for: inspecting a dataset found with arcgis_pe_search_datasets
...
@tool
async def arcgis_pe_query_feature_layer(
    item_id: str,
    ...
    """Query rows from one Prince Edward Island FeatureServer/MapServer layer.

    Use for: reading actual feature attribute values (not just
    metadata) from a dataset found with arcgis_pe_search_datasets,
