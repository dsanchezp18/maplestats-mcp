"""MCP tools for Data MB (Manitoba)."""
...
from maple_data_mcp.modules.arcgis_mb.constants import ROWS_LIMIT_DEFAULT, SEARCH_LIMIT_DEFAULT
from maple_data_mcp.modules.arcgis_mb.schemas import (
    DatasetSearchResult,
    FeatureQueryResult,
    ItemDetail,
)
...
@tool
async def arcgis_mb_search_datasets(
    query: str = "",
    tag: str | None = None,
    ...
    """Search Manitoba's ArcGIS Hub open-data catalogue (Data MB).

    Use for: finding Manitoba datasets by topic, tag, item type, or
...
@tool
async def arcgis_mb_get_dataset(item_id: str, lang: Lang = "en") -> ItemDetail:
    """Get one Manitoba dataset item's metadata, service URL, and download links.

    Use for: inspecting a dataset found with arcgis_mb_search_datasets
    before querying its rows or downloading it. Keywords: Manitoba,
    Data MB, ArcGIS Hub, dataset detail, FeatureServer, MapServer,
    metadata, licence, download, CSV, shapefile, GeoJSON, KML.
    Mots-clés : Manitoba, Data MB, ArcGIS Hub, détail du jeu de
...
@tool
async def arcgis_mb_query_feature_layer(
    item_id: str,
    ...
    """Query rows from one Manitoba FeatureServer/MapServer layer.

    Use for: reading actual feature attribute values (not just
    metadata) from a dataset found with arcgis_mb_search_datasets,
    ...
    first layer or table id (not always 0). Keywords: Manitoba, Data MB,
    ArcGIS REST, FeatureServer, MapServer, query, rows, attributes,
    filter, where clause, geospatial, GIS.
    Mots-clés : Manitoba, Data MB, ArcGIS REST, FeatureServer,
