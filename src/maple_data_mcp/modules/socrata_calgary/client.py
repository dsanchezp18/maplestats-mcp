"""HTTP client for the City of Calgary's Socrata (SODA) platform.
...
from maple_data_mcp.modules.socrata_calgary import constants
...
async def search_datasets(
    ...
    *,
    ...
    """Search Calgary's dataset catalogue; ``lang`` is accepted for consistency."""
    del lang
    ...
    cache_key = f"socrata-calgary:search:{query}:{category}:{tag}:{limit}:{offset}"
    ...
    return DatasetSearchResult(
        ...
        provenance=make_provenance(
            ...
            cached=was_cached,
            schema_name="socrata_calgary.DatasetSearchResult",
...
async def get_dataset(dataset_id: str, lang: str = "en") -> DatasetDetail:
    ...
    obj, was_cached = await cached_fetch(
        f"socrata-calgary:get_view:{dataset_id}", constants.CACHE_TTL_DATASET_SECONDS, fetch
    )
    ...
    return DatasetDetail(
        ...
        provenance=make_provenance(
            ...
            cached=was_cached,
            schema_name="socrata_calgary.DatasetDetail",
        ),
    )
...
async def query_dataset_rows(
    ...
    *,
    ...
    cache_key = f"socrata-calgary:rows:{dataset_id}:{select}:{where}:{order}:{q}:{limit}:{offset}"
    ...
    return RowQueryResult(
        ...
        provenance=make_provenance(
            ...
            cached=was_cached,
            schema_name="socrata_calgary.RowQueryResult",
...
async def list_categories(lang: str = "en") -> CategoryList:
    ...
    raw, was_cached = await cached_fetch(
        "socrata-calgary:domain_categories", constants.CACHE_TTL_FACET_SECONDS, fetch
    )
    categories = [
    ...
    return CategoryList(
        ...
        provenance=make_provenance(
            ...
            cached=was_cached,
            schema_name="socrata_calgary.CategoryList",
        ),
    )
...
async def list_tags(lang: str = "en") -> TagList:
    ...
    raw, was_cached = await cached_fetch(
        "socrata-calgary:domain_tags", constants.CACHE_TTL_FACET_SECONDS, fetch
    )
    tags = [
    ...
    return TagList(
        ...
        provenance=make_provenance(
            ...
            cached=was_cached,
            schema_name="socrata_calgary.TagList",
        ),
    )
