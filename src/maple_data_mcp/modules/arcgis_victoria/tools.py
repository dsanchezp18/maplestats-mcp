"""MCP tools for the City of Victoria's ArcGIS Hub open-data portal."""
...
from maple_data_mcp.modules.arcgis_victoria import client
from maple_data_mcp.modules.arcgis_victoria.constants import (
    ROWS_LIMIT_DEFAULT,
...
@tool
async def arcgis_victoria_search_datasets(
    query: str = "",
    tag: str | None = None,
    ...
    """Search Victoria, BC's ArcGIS Hub open-data catalogue (VicMap).

    Use for: finding Victoria datasets by topic, tag, item type, or
...
@tool
async def arcgis_victoria_get_dataset(item_id: str, lang: Lang = "en") -> ItemDetail:
    """Get one Victoria dataset item's metadata, service URL, and download links.

    Use for: inspecting a dataset found with arcgis_victoria_search_datasets
...
@tool
async def arcgis_victoria_query_feature_layer(
    item_id: str,
    ...
    """Query rows from one Victoria FeatureServer/MapServer layer.

    Use for: reading actual feature attribute values (not just
    metadata) from a dataset found with arcgis_victoria_search_datasets,
