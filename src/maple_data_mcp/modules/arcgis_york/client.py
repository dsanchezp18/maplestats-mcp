"""HTTP client for the York Region Open Data, an ArcGIS Hub deployment.
...
from maple_data_mcp.modules.arcgis_york import constants
...
async def search_datasets(
    ...
    *,
    ...
    """Search York Region's ArcGIS Hub dataset catalogue; ``lang`` is accepted for consistency."""
    del lang
    ...
    cache_key = f"arcgis-york:search:{query}:{tag}:{item_type}:{limit}:{offset}"
    ...
    return DatasetSearchResult(
        ...
        provenance=make_provenance(
            ...
            cached=was_cached,
            schema_name="arcgis_york.DatasetSearchResult",
...
async def get_dataset(item_id: str, lang: str = "en") -> ItemDetail:
    """Get one York Region dataset item's metadata and download links; ``lang`` is a documented no-op."""
    del lang
    ...
    feature, was_cached = await cached_fetch(
        f"arcgis-york:get_item:{item_id}", constants.CACHE_TTL_ITEM_SECONDS, fetch
    )
    props = feature.get("properties") or {}
    ...
    return ItemDetail(
        ...
        provenance=make_provenance(
            ...
            cached=was_cached,
            schema_name="arcgis_york.ItemDetail",
        ),
    )
...
async def query_feature_layer(
    ...
    *,
    ...
    """Query rows from one York Region FeatureServer/MapServer layer; ``lang`` is accepted for consistency.
    ...
    cache_key = (
        f"arcgis-york:query:{item_id}:{resolved_layer_index}:{where_clause}:{out_fields}:"
    ...
    return FeatureQueryResult(
        ...
        provenance=make_provenance(
            ...
            cached=was_cached,
            schema_name="arcgis_york.FeatureQueryResult",
            limits=(
