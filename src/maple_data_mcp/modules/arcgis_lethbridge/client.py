"""HTTP client for the Lethbridge Open Data Catalogue, an ArcGIS Hub deployment.
...
from maple_data_mcp.modules.arcgis_lethbridge import constants
...
async def search_datasets(
    ...
    *,
    ...
    """Search Lethbridge's ArcGIS Hub dataset catalogue; ``lang`` is accepted for consistency."""
    del lang
    ...
    cache_key = f"arcgis-lethbridge:search:{query}:{tag}:{item_type}:{limit}:{offset}"
    ...
    return DatasetSearchResult(
        ...
        provenance=make_provenance(
            ...
            cached=was_cached,
            schema_name="arcgis_lethbridge.DatasetSearchResult",
...
async def get_dataset(item_id: str, lang: str = "en") -> ItemDetail:
    """Get one Lethbridge dataset item's metadata and download links; ``lang`` is a documented no-op."""
    del lang
    ...
    feature, was_cached = await cached_fetch(
        f"arcgis-lethbridge:get_item:{item_id}", constants.CACHE_TTL_ITEM_SECONDS, fetch
    )
    props = feature.get("properties") or {}
    ...
    return ItemDetail(
        ...
        provenance=make_provenance(
            ...
            cached=was_cached,
            schema_name="arcgis_lethbridge.ItemDetail",
        ),
    )
...
async def query_feature_layer(
    ...
    *,
    ...
    """Query rows from one Lethbridge FeatureServer/MapServer layer; ``lang`` is accepted for consistency.
    ...
    cache_key = (
        f"arcgis-lethbridge:query:{item_id}:{resolved_layer_index}:{where_clause}:{out_fields}:"
    ...
    return FeatureQueryResult(
        ...
        provenance=make_provenance(
            ...
            cached=was_cached,
            schema_name="arcgis_lethbridge.FeatureQueryResult",
            limits=(
