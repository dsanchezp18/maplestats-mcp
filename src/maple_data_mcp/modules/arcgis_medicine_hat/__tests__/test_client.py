...
from maple_data_mcp.modules.arcgis_medicine_hat import client, constants
...
_SEARCH_FEATURE = {
    "id": "4820ab127fc64f338029e7790e7450c0",
    "type": "Feature",
    "geometry": None,
    "properties": {
        "id": "4820ab127fc64f338029e7790e7450c0",
        "title": "Park Washrooms",
        "snippet": "Locations and attributes of public washrooms maintained by the City of Medicine Hat Parks.",
        "type": "Feature Service",
        "tags": ["Parks", "Medicine Hat"],
        "categories": [],
        "owner": "MHArcGISOnline",
        "modified": 1777933348000,
        "numViews": 229,
    },
}

_ITEM_DETAIL_FEATURE = {
    "id": "4820ab127fc64f338029e7790e7450c0_0",
    "type": "Feature",
    "geometry": None,
    "properties": {
        "id": "4820ab127fc64f338029e7790e7450c0",
        "title": "Park Washrooms",
        "description": "Locations and attributes of public washrooms maintained by the City of Medicine Hat Parks.",
        "type": "Feature Service",
        "tags": ["Parks", "Medicine Hat"],
        "categories": [],
        "owner": "MHArcGISOnline",
        "licenseInfo": "<p>City of Medicine Hat Open Data Licence.</p>",
        "created": 1769186129000,
        "modified": 1777933348000,
        "numViews": 229,
        "spatialReference": "102100",
        "url": "https://services3.arcgis.com/2gQ2Dv4Fb016qFVU/arcgis/rest/services/Park_Washrooms/FeatureServer",
    },
}
...
async def test_search_parses_hub_search_fields(httpx_mock):
    ...
    assert result.items[0].id == "4820ab127fc64f338029e7790e7450c0"
    assert result.items[0].item_type == "Feature Service"
    assert result.items[0].landing_page_url.endswith("/datasets/4820ab127fc64f338029e7790e7450c0")
...
async def test_get_dataset_maps_item_detail_and_download_links(httpx_mock):
    httpx_mock.add_response(
        url=f"{_COLLECTION_URL}/items/4820ab127fc64f338029e7790e7450c0",
        json=_ITEM_DETAIL_FEATURE,
    )
    result = await client.get_dataset("4820ab127fc64f338029e7790e7450c0")
    assert result.id == "4820ab127fc64f338029e7790e7450c0"
...
async def test_query_feature_layer_returns_attributes(httpx_mock):
    ...
    service_url = "https://services3.arcgis.com/2gQ2Dv4Fb016qFVU/arcgis/rest/services/Park_Washrooms/FeatureServer"
    httpx_mock.add_response(
        url=f"{service_url}?f=json", json={"layers": [{"id": 12}], "tables": []}
    )
    httpx_mock.add_response(
        url=(
            f"{service_url}/12/query?where=1%3D1&outFields=%2A&f=json"
    ...
    result = await client.query_feature_layer("4820ab127fc64f338029e7790e7450c0")
...
async def test_feature_service_embedded_error_becomes_invalid_input(httpx_mock):
    ...
    httpx_mock.add_response(
        url=(
            f"{service_url}/12/query?where=garbage%3D%3D&outFields=%2A&f=json"
            "&resultRecordCount=10&resultOffset=0&returnGeometry=false"
        ),
        json={"error": {"code": 400, "message": "Invalid query.", "details": ["bad where"]}},
    )
    with pytest.raises(InvalidInput):
        await client.query_feature_layer("4820ab127fc64f338029e7790e7450c0", where="garbage==")
