"""HTTP client for Open Data Nova Scotia's Socrata (SODA) platform.
...
from maple_data_mcp.modules.socrata_ns import constants
...
async def search_datasets(
    ...
    *,
    ...
    """Search Nova Scotia's dataset catalogue; ``lang`` is accepted for consistency."""
    del lang
    ...
    cache_key = f"socrata-ns:search:{query}:{category}:{tag}:{limit}:{offset}"
    ...
    return DatasetSearchResult(
        ...
        provenance=make_provenance(
            ...
            cached=was_cached,
            schema_name="socrata_ns.DatasetSearchResult",
...
async def get_dataset(dataset_id: str, lang: str = "en") -> DatasetDetail:
    ...
    obj, was_cached = await cached_fetch(
        f"socrata-ns:get_view:{dataset_id}", constants.CACHE_TTL_DATASET_SECONDS, fetch
    )
    ...
    return DatasetDetail(
        ...
        provenance=make_provenance(
            ...
            cached=was_cached,
            schema_name="socrata_ns.DatasetDetail",
        ),
    )
...
async def query_dataset_rows(
    ...
    *,
    ...
    cache_key = f"socrata-ns:rows:{dataset_id}:{select}:{where}:{order}:{q}:{limit}:{offset}"
    ...
    return RowQueryResult(
        ...
        provenance=make_provenance(
            ...
            cached=was_cached,
            schema_name="socrata_ns.RowQueryResult",
...
async def list_categories(lang: str = "en") -> CategoryList:
    ...
    raw, was_cached = await cached_fetch(
        "socrata-ns:domain_categories", constants.CACHE_TTL_FACET_SECONDS, fetch
    )
    categories = [
    ...
    return CategoryList(
        ...
        provenance=make_provenance(
            ...
            cached=was_cached,
            schema_name="socrata_ns.CategoryList",
        ),
    )
...
async def list_tags(lang: str = "en") -> TagList:
    ...
    raw, was_cached = await cached_fetch(
        "socrata-ns:domain_tags", constants.CACHE_TTL_FACET_SECONDS, fetch
    )
    tags = [
    ...
    return TagList(
        ...
        provenance=make_provenance(
            ...
            cached=was_cached,
            schema_name="socrata_ns.TagList",
        ),
    )
