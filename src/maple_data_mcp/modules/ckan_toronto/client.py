...
async def datastore_search(
    ...
    *,
    ...
    (from ckan_toronto_get_dataset/ckan_toronto_get_resource) before calling this.
    """
    del lang
    if not resource_id.strip():
        raise InvalidInput("resource_id must not be empty.")
    ...
    cache_key = f"ckan-toronto:datastore_search:{resource_id}:{filters}:{query}:{sort}:{fields}:{limit}:{offset}"
    ...
    return DatastoreSearchResult(
        ...
        provenance=make_provenance(
            ...
            cached=was_cached,
            schema_name="ckan_toronto.DatastoreSearchResult",
